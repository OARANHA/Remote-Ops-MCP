...
export function adminRouter():Router{
  ...
  r.post("/revoke-all",requireCsrf,(_req,res)=>{const count=revokeAllSessions();res.redirect(303,`/admin?notice=${encodeURIComponent(`${count} sessão(ões) revogada(s).`)}`);});
  r.post("/clients/:clientId/revoke",requireCsrf,(req,res)=>{const ok=revokeClient(req.params.clientId);res.redirect(303,`/admin?notice=${encodeURIComponent(ok?"Cliente revogado imediatamente.":"Cliente não encontrado.")}`);});
  r.post("/sessions/:sessionId/revoke",requireCsrf,(req,res)=>{const ok=revokeSession(req.params.sessionId);res.redirect(303,`/admin?notice=${encodeURIComponent(ok?"Sessão revogada imediatamente.":"Sessão não encontrada.")}`);});
  r.post("/targets/:targetId/revoke",requireCsrf,(req,res)=>{const t=getTarget(req.params.targetId);if(t){setTargetEnabled(t.id,false,"revogado no console administrativo");closeTargetPool(t.id);}res.redirect(303,`/admin?notice=${encodeURIComponent(t?"Device revogado. Novas operações estão bloqueadas.":"Device não encontrado.")}`);});
  r.post("/targets/:targetId/enable",requireCsrf,(req,res)=>{const t=getTarget(req.params.targetId);if(t)setTargetEnabled(t.id,true,"habilitado no console administrativo");res.redirect(303,`/admin?notice=${encodeURIComponent(t?"Device habilitado.":"Device não encontrado.")}`);});
