...
r="$(mcp 3111 '{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}')"; n="$(python3 -c 'import json,sys; print(len(json.load(sys.stdin)["result"]["tools"]))' <<<"$r")"; [ "$n" = 38 ] || { echo "FAIL: expected 38 tools, got $n"; exit 1; }; ok '38 capability-scoped tools exposed'
