...
mcp(){ local port="$1" body="$2" [REDACTED]"; local auth=(); [ -n "$token" ] && auth=(-H "Authorization: Bearer $token"); curl -fsS -X POST "http://127.0.0.1:$port/mcp" -H 'Content-Type: application/json' -H 'Accept: application/json, text/event-stream' "${auth[@]}" -d "$body"; }
...
r="$(mcp 3111 '{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}')"; n="$(python3 -c 'import json,sys; print(len(json.load(sys.stdin)["result"]["tools"]))' <<<"$r")"; [ "$n" = 39 ] || { echo "FAIL: expected 39 tools, got $n"; exit 1; }; ok '39 capability-scoped tools exposed'
...
r="$(mcp 3111 '{"jsonrpc":"2.0","id":6,"method":"tools/call","params":{"name":"docker_logs","arguments":{"target":"demo-mock","container":"wandora-web","lines":20}}}')"; contains "$r" '[REDACTED]' 'secret redaction active'; not_contains "$r" '[REDACTED]' 'raw API key not leaked'
stop_last
...
redir="$(curl -sS -o /dev/null -w '%{redirect_url}' -X POST http://127.0.0.1:3112/authorize/submit --data-urlencode "[REDACTED]" --data-urlencode 'response_type=code' --data-urlencode "client_id=$cid" --data-urlencode 'redirect_uri=http://127.0.0.1/callback' --data-urlencode 'state=e2e-state' --data-urlencode "code_challenge=$challenge" --data-urlencode 'code_challenge_method=S256' --data-urlencode 'scope=mcp:read offline_access')"
...
r="$(curl -fsS -X POST http://127.0.0.1:3112/token -H 'Content-Type: application/x-www-form-urlencoded' --data-urlencode 'grant_type=authorization_code' --data-urlencode "code=$authcode" --data-urlencode "code_verifier=$verifier" --data-urlencode "client_id=$cid" --data-urlencode 'redirect_uri=http://127.0.0.1/callback')"; [REDACTED] -c 'import json,sys; print(json.load(sys.stdin)["access_token"])' <<<"$r")"; refresh="$(python3 -c 'import json,sys; print(json.load(sys.stdin)["refresh_token"])' <<<"$r")"; [ -n "$token" ] && [ -n "$refresh" ] || exit 1; ok 'access + refresh token issued'
...
jar="$TMP/admin.cookie"; curl -fsS -c "$jar" -X POST http://127.0.0.1:3112/admin/login --data-urlencode "[REDACTED]" -o /dev/null
